...
const SRC =
  'C:/Users/copan/.cursor/projects/c-Users-copan-Desktop-Once-Upon-a-Scam/assets/c__Users_copan_AppData_Roaming_Cursor_User_workspaceStorage_39f0117879db467f92618b6dc4c58c15_images_Mad_Woodsman-95032694-c6e5-4b72-bb49-709407f10e7f.jpg';
const OUT_DIR = 'C:/Users/copan/Desktop/Once Upon a Scam/public/art/units/mad-woodsman';
const CARD_W = 642;
const CARD_H = 972;
const KEY = [0, 254, 102];
...
function scaleAboutInner(img, inner, tw, th) {
  const scale = Math.min(tw / (inner.r - inner.l + 1), th / (inner.b - inner.t + 1));
  const dw = Math.max(1, Math.round(img.width * scale));
  const dh = Math.max(1, Math.round(img.height * scale));
  return scaleTo(img, dw, dh);
}
...
writePng(sized, `${OUT_DIR}/card-cutout-preview.png`);
writePng(checker(sized), `${OUT_DIR}/card-cutout-check.png`);

console.log(
  JSON.stringify(
    {
      src: `${framed.width}x${framed.height}`,
      full,
      inner,
      innerSize: `${inner.r - inner.l + 1}x${inner.b - inner.t + 1}`,
      cropped: `${cropped.width}x${cropped.height}`,
      out: `${sized.width}x${sized.height}`,
    },
    null,
    2,
  ),
